  ...
  const toggleVoiceInput = useCallback(() => {
    ...
    if (!SpeechRecognition) {
      setGlobalNotice({
        ...
        type: 'warning',
        message: 'La reconnaissance vocale n’est pas disponible sur ce navigateur / système.',
      });
      return;
    }
    ...
    try {
      ...
      recognition.lang = currentLang.startsWith('fr') ? 'fr-FR' : currentLang.startsWith('zh') ? 'zh-CN' : 'en-US';
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-0.5 mt-1">
                        {(() => {
                          ...
                          const activeProfile = activeProfileKey
                            ? appConfig?.profiles?.[activeProfileKey as keyof typeof appConfig.profiles]
                            : undefined;
                          ...
                          // Toujours inclure le modèle actif s'il n'est pas déjà dans la liste
